...
boolean SID_SA_SUBID_RSD_Callback(uint8_t *pInd, uint8_t *pRes, uint32_t *pResLen)
{
	...
	if (Uds_SID_SA_Info.AccessDelay == 0U)
	{
		...
		{
			Uds_Seed[0U] = 0x11;//(uint8_t)(((((((cur_STM_TIM0 - 1) * 23) * 1364271484))) / 3 >> 11) % 1268735 & 0xFF);
			Uds_Seed[1U] = 0x11;//(uint8_t)((((((cur_STM_TIM0 * 27145826921) * 32584)) / 97) * 61 >> 8) % 71 & 0xFF);
			Uds_Seed[2U] = 0x11;//(uint8_t)((((((cur_STM_TIM0 * 27145826921) * 32584)) / 97) * 61 >> 11) % 71 & 0xFF);
			Uds_Seed[3U] = 0x11;//(uint8_t)((cur_STM_TIM0) & 0xFF);
		}
