...
void canfd0_callback(can_callback_args_t * p_args)
{
    switch (p_args->event)
    {
        ...
        {
            canfd0_rx_complete_flag = true; //canfd1接收到数据

            /* 读取接收帧 */
            memcpy(&canfd0_rx_frame, &(p_args->frame), sizeof(can_frame_t));
			//uds_recv_frame(canfd0_rx_frame.id, canfd0_rx_frame.data, 8);
			//uds_recv_frame(canfd0_rx_frame.id, canfd0_rx_frame.data, canfd0_rx_frame.data_length_code);
...
void CAN0_Operation(void)
{
    ...
    CANFD1_MSG_PRINTF("CANFD0 正在使用传统 CAN 帧传输数据");

    /* 通过 mail box #0 传输数据 */
    err = g_canfd_on_canfd.write(&g_canfd0_ctrl, CAN_MAILBOX_NUMBER_0, &canfd0_tx_frame);
    assert(FSP_SUCCESS == err);
...
void CANFD0_Operation(void)
{
    ...
    CANFD1_MSG_PRINTF("CANFD0 正在使用 CAN FD 帧传输数据");
...
void CAN0_SendMessage(uint32_t U32_transmitCANid, uint8_t *U8_dataarr, uint8_t U8_DLC)
{
    ...
    /* 更新传输帧的参数 */
    canfd0_tx_frame.id = U32_transmitCANid;
    canfd0_tx_frame.id_mode = CAN_ID_MODE_STANDARD;
    canfd0_tx_frame.type = CAN_FRAME_TYPE_DATA;
    canfd0_tx_frame.data_length_code = U8_DLC;
    canfd0_tx_frame.options = 0;

    /* 填充将要传输的帧数据 */
    memcpy(canfd0_tx_frame.data, U8_dataarr, canfd0_tx_frame.data_length_code);
...
void CANFD0_SendMessage(uint32_t U32_transmitCANid, uint8_t *U8_dataarr, uint8_t U8_DLC)
{
    ...
    {
        canfd0_tx_frame.data[j] = (uint8_t) U8_dataarr[j];
    }


    //CANFD1_MSG_PRINTF("CANID: 0x%x 传输CANFD数据",canfd1_tx_frame.id);
...
/*
    ...
    MsgPtr.id = CAN_ID;
    MsgPtr.id_mode = CAN_ID_MODE_EXTENDED;
    MsgPtr.type = CAN_FRAME_TYPE_DATA;
    MsgPtr.data_length_code = CAN_FD_DATA_LENGTH_CODE;
    MsgPtr.options = CANFD_FRAME_OPTION_FD | CANFD_FRAME_OPTION_BRS;
    for( uint16_t j = 0; j < CAN_FD_DATA_LENGTH_CODE; j++)
    {
        MsgPtr.data[j] = (uint8_t) (j);
    }
*/
    ...
    /* 通过 mail box #0 传输数据 */
    err = R_CANFD_Write(&g_canfd0_ctrl, CAN_MAILBOX_NUMBER_0, &MsgPtr);
    assert(FSP_SUCCESS == err);
