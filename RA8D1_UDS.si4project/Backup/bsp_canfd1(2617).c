...
void CANFD1_Operation(void)
{
    ...
    {
        canfd1_tx_frame.data[j] = (uint8_t) (j);
    }
...
void HLI_CanFD_Write(HLI_Canfd_MessageType MsgPtr)
{
    ...
    /* 更新 FD 帧的参数 */
    canfd1_tx_frame.id = MsgPtr.ID;
    canfd1_tx_frame.id_mode = MsgPtr.IDType;
    canfd1_tx_frame.type = CAN_FRAME_TYPE_DATA;
    canfd1_tx_frame.data_length_code = MsgPtr.DataLength;
    canfd1_tx_frame.options = CANFD_FRAME_OPTION_FD | CANFD_FRAME_OPTION_BRS;
	for( uint16_t j = 0; j < CAN_FD_DATA_LENGTH_CODE; j++)
	{
		canfd1_tx_frame.data[j] = *(MsgPtr.DataPtr+j);
	}
